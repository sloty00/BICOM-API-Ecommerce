...
const getMdEcommP = async (bd_name, host, page) => {//Funcion de tipo asincronica.
    ...
    const ecomps = await queryEcommp(page, mysql, limit, offset)
    let jsonResult = {
        'total_rows': total_elementos,
        'total_page': total_paginas,
        'number_pagination': ecomps.length,
        'page': page,
        'data': ecomps
    }
    return jsonResult;
...
const queryEcommp = async (page, mysql, limit, offset) => {//Funcion de tipo asincronica, realiza la consulta.
    ...
    const totalQuery = "SELECT COUNT(*) AS id FROM (ecommerce_parameters AS ecommp INNER JOIN sellers as sell ON ecommp.seller_id = sell.id) INNER JOIN warehouses as ware ON ecommp.warehouse_id = ware.id "
