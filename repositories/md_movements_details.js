...
const getMdMovement = async (bd_name, host, page, doc_type, is_sii, consolidate, invoice_date, begin_date, end_date) => {//Funcion de tipo asincronica.
  ...
  switch (doc_type == "" && doc_type) {
    ...
    case '1':
        filter_doc_type="WHERE mov.document_type_id= 1 ";
        console.log('Boletas Afecta');
    break;

    case '2':
        filter_doc_type="WHERE mov.document_type_id= 2 ";
        console.log('Boletas Exenta');
    break;

    case '3':
        filter_doc_type="WHERE mov.document_type_id= 3 ";
        console.log('Facturas Afecta');
    break;
    
    case '4':
        filter_doc_type="WHERE mov.document_type_id= 4 ";
        console.log('Facturas Exenta');
    break;

    case '7':
        filter_doc_type="WHERE mov.document_type_id= 7 ";
        console.log('Notas de Debito');
    break;

    case '8':
        filter_doc_type="WHERE mov.document_type_id= 8 ";
        console.log('Notas de Credito');
    break;

    case '10':
        filter_doc_type="WHERE mov.document_type_id= 10 ";
        console.log('Guias de Despacho');
    break;

    case '11':
        filter_doc_type="WHERE mov.document_type_id= 11 ";
  ...
  switch (filter_doc_type !== "" && is_sii !=="" && is_sii) {
  ...
  switch (filter_doc_type == "" && is_sii !=="" && is_sii) {
    ...
    case '1':
        filter_is_sii="WHERE mov.is_sii = 1";
        console.log('Electronica:'.is_sii);
    break;

    case '2':
        filter_is_sii="WHERE mov.is_sii = 2";
        console.log('No Electronico'.is_sii);
    break;
  }

  switch (filter_doc_type == "" && filter_is_sii == "" && consolidate !=="" && consolidate) {

    case '0':
        filter_consolidate="WHERE mov.consolidate = 0";
        console.log('Pendientes');
    break;

    case '1':
        filter_consolidate="WHERE mov.consolidate = 1";
        console.log('Anulada');
    break;

    case '2':
        filter_consolidate="WHERE mov.consolidate = 2";
  ...
  switch (filter_doc_type =="" && invoice_date !=="" && invoice_date) {
    case invoice_date:
        filter_invoice_date="WHERE mov.invoice_date = " + `'${invoice_date}'`;
        console.log('comillas1: ' + filter_invoice_date);
    break;
  }
  ...
  switch (filter_doc_type == "" && begin_date !=="" && begin_date) {
    case begin_date:
        filter_begin_date="WHERE mov.invoice_date >= " + `'${begin_date}'`;
        console.log('Repository: '+begin_date);
    break;
  }

  switch (filter_doc_type == "" && end_date !=="" && end_date) {
    case end_date:
